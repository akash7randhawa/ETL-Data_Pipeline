...
client = boto3.client('emr', region_name='us-east-1',aws_access_key_id='AKIA5NLRO3JUUWB4Q4PM',aws_secret_access_key='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')
...
def create_emr_cluster():
  cluster_id = client.run_job_flow(
    ...
    'Ec2SubnetId': 'subnet-099d7771c6e498ecd',
    },
    LogUri="s3://sample-bucket-for-airflow/",
...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://irisdata3447/scripts/ingest.sh']],
  dag=dag, 
  )
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
            '--master', 'yarn',
            '--deploy-mode', 'cluster',
            's3://irisdata3447/scripts/transform.py']],
  dag=dag, 
  )
